...
def index(request):
    context = {}
    return render(request, 'StorageS/index.html', context)


@login_required
def clients(request):
    context = {
        'clients_list': Client.objects.raw("Select * from StorageS_client"),
    }
    return render(request, 'StorageS/clients.html', context)
...
def products(request):
    ...
    if request.POST:
        ...
        context["color"] = rest
        return render(request, 'StorageS/products.html', context)
    return render(request, 'StorageS/products.html', context)
...
def result(request):
    ...
    if request.POST:
        ...
        return render(request, 'StorageS/result.html', context)
    return render(request, 'StorageS/result.html', context)
...
def new_order(request):
    ...
    if request.POST:
        ...
        return render(request, 'StorageS/new_order.html', context)
    return render(request, 'StorageS/new_order.html', context)
...
def basket(request):
    context = {}
    return render(request, "StorageS/basket.html", context)
...
def next_basket(request):
    if request.POST:
        if request.POST.getlist('product_count_b'):
            ...
            # print(sess)
            request.session['basket'] = sess
    return render(request, "StorageS/next_basket.html", {})


@login_required
def add_product(request):
    context = {}
    return render(request, "StorageS/add_product.html", context)
    ...
    if request.POST:
        ...
        if user is not None:
            if user.username == 'admin':
                ...
                branch_dict = {'branch': Branch.objects.raw(
                    """call get_branch(%s)""", [user])}
                request.session['man_id'] = man_dict['man_id'][0].id
                request.session['branch_id'] = branch_dict['branch'][0].id
                request.session['branch_address'] = branch_dict[
                    'branch'][0].branch_address
                auth.login(request, user)
                return redirect('/storage/')
        else:
            context = {'error': "Введите правильный логин или пароль."}
            return render(request, 'StorageS/login.html', context)
    return render(request, 'StorageS/login.html', context)
